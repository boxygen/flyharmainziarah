...
.card-item .card-title{max-height: 16px;}
</style>
...
form.append("checkin", "2022-08-25");
form.append("checkout", "2022-08-25");
form.append("nationality", "IN");
form.append("adults", "2");
form.append("chlids", "0");
form.append("child_ages", "[{ages:5},{ages:6}]");
form.append("rooms", "1");
form.append("lang", "en");
form.append("currency", "USD");
form.append("ip", "::1");
form.append("browser_version", "chrome");
form.append("type", "postman");
form.append("os", "windows");

var settings = {
  "url": "<?=api_url?>api/hotel/search?appKey=phptravels",
  "method": "POST",
  "timeout": 0,
  "processData": false,
              ...
              <div class="left-side-info rtl-align-right" style="color:#fff">
                <span>
                  <strong style="text-transform: capitalize">
                    <h2 class="sec__title_list"><?= T::hotels_search_hotels ?> <?= T::in ?> <?=$city?></h2>
                  </strong>
                </span>
                <div>
                  <p><strong><?=$nights?> <?= T::hotels_nights ?> </strong>( <?=$checkin?> - <?=$checkout?> )</p>
                  <p><strong><?=$adults?></strong> <?= T::hotels_adults ?> <strong><?=$childs?></strong> <?= T::hotels_childs ?> <strong><?=$rooms?></strong> <?=T::hotels_rooms?></p>
                 </div>
              </div>
            </div>
          </div>
        </div>
        <?php if ( empty($hotels_data) ) { } else {  ?>
        <div class="col-lg-4">
          <div class="breadcrumb-list d-flex gap-2 accordion">
            <ul class="list-items d-flex justify-content-end d-none d-sm-block">
              <li class="d-flex justify-content-center align-items-center h-100"><i class="la la-hotel"></i> <?=T::total?> <strong><?=count($hotels_data)?></strong>  <?=T::hotels_hotelsin?> <?=$city?></li>
            </ul>
...
<div class="linear linear_listing d-none">
    ...
    <div class="col-md-3 d-sm-block search_filter_options" id="fadein" style="display: none">
            ...
            <form>
              ...
              <div class="sidebar-widget" data-filter-group>
                ...
                <ul class="list remove_duplication" data-filter-group>
                  <!--<?php foreach ($hotels_data as $index => $item) { ?>
                    <li>
                        <div class="custom-checkbox flights_line">
                            <input class="filter" type="checkbox" id="stars<?=$item->stars?>" data-filter=".stars<?=$item->stars?>">
                            <label for="stars<?=$item->stars?>"> <?=$item->stars?></label>
                        </div>
                    </li>
                    <?php } ?>-->
                  <!-- 1 star -->
      ...
      <!-- <?= signupdeals();?> -->
        ...
        <section data-ref="container" id="data">
          <ul>
            <?php foreach ($hotels_data as $index => $item) {
            if (empty($item->city_code)) { $item->city_code = "0"; }
            if (empty($item->country_code)) { $item->country_code = "0"; }
            $link = root.'hotel/'.$session_lang.'/'.strtolower($currency).'/'.strtolower(str_replace(' ', '-', $city)).'/'.strtolower(str_replace(" ", "-", $item->name)).'/'.$item->hotel_id.'/'.$checkin.'/'.$checkout.'/'.$rooms.'/'.$adults.'/'.$childs.'/'.$item->supplier.'/'.$nationality.'/'.strtoupper($item->country_code).'/'.$item->city_code.'/';
            ...
            ?>
            <li class="mix stars_<?=$item->stars?> hotels_amenities_" data-a="<?=$item->price?>" data-b="" id="<?=strtolower($item->name)?>">
              <div class="card-item card-item-list" class="marker-link row row-rtl item hotelslist">
                <div class="card-img p-2">

                  <a href="<?=$link?>" class="d-block">
                  <img data-src="<?=$hotel_img?>" loading="lazy" class="main-img lazyload" alt="thumbnail"  data-expand="-20" />
                  </a>
                ...
                <div class="card-body p-0">
                  ...
                  <h3 class="card-title" style="max-height: 26px !important;"><?=$item->name?>
                  ...
                  <small style="text-overflow: ellipsis; overflow: hidden;white-space: pre;font-size: 12px;"><?=substr($item->address, 0, 50);?> </small>

                  <p class="card-meta"><i class="la la-map-marker"></i> <?= $_SESSION['hotels_location'] // $item->location?></p>
                  ...
                  <a class="ellipsisFIX go-text-right mob-fs14" href="javascript:void(0);" onclick="showMap('http://localhost/v8/home/maps/-31.95819269999999/115.86670630000003/hotels/38','modal');" title="dubai">
                  </a>

                  <div class="card-rating pt-0 pb-0">
                    <!--<span class="badge text-white"><?=$item->stars?></span>-->
                  ...
                  <div class="col-4 p-2" style="background:#fff">
                    <div class="card-price bg-white">
                      ...
                      <p class="mb-0"><span class="price__num"><small><?=$currency?></small> <strong><?=number_format($item->b2b_price,2)?> <span class="prices"><?=T::agent_price?></span></strong><div class="clear"></div></span></p>
                      <!--<p class="mb-0"><span class="price__num"><small><?=$currency?></small> <strong><?=number_format($item->b2c_price,2)?> <span class="prices"><?=T::customer_price?></span></strong><div class="clear"></div></span></p>-->
                      <?php } else { ?>
                      <p class="mb-0"><span class="price__num"><small><?=$currency?></small> <strong><?=number_format($item->b2c_price,2)?></strong></span></p>
                      <?php } } ?>
                      ...
                      <div class="clear"></div>
                      <p class="mb-4"><?=$nights?> <?=T::hotels_nights?></p>
                      <?php if (empty($item->redirect)) {?>
                        ...
                        <?php } else { ?>
                          <a href="<?=$item->redirect?>" class="more_details effect mt-0 btn-block" data-style="zoom-in" target="_blank">
                          <?=T::details?><i class="la la-angle-right"></i>
                        </a>
                        <?php } ?>
...
<button id="search_filter" class="btn btn-warning btn-lg btn-block btn_filter_search">
