  ...
  <div class="breadcrumb-wrap">
    <div class="container">
      <div class="row align-items-center">
              ...
              <div class="left-side-info rtl-align-right" style="color:#fff">
                <span>
                  <strong style="text-transform: capitalize">
                    <h2 class="sec__title_list"><?= T::tours_search_tours ?> <?= T::in ?> <?=$from_location?></h2>
                  </strong>
                </span>
                <div>
                  <p><strong><?= T::date ?> </strong>( <?=$fromdate?> )</p>
        ...
        <?php if ( empty($cars_data) ) { } else {  ?>
        <div class="col-lg-5">
         <div class="breadcrumb-list d-flex gap-2 accordion">
            <ul class="list-items d-flex justify-content-end d-none d-sm-block">
              <li class="d-flex justify-content-center align-items-center h-100"><i class="la la-briefcase"></i> <?= T::total?> <strong><?= count($cars_data)?></strong>  <?= T::tours_tours_in?> <?=$from_location?></li>
            </ul>
...
<section>
  <div class="cd-main-content container mt-4">
    ...
    <div class="col-md-3 d-none search_filter_options" id="fadein" style="display: none">
      ...
      <!-- end col-lg-4 -->
      <div class="col-lg-12" id="markers">
      ...
      <!-- <?= signupdeals();?> -->
        ...
        <section data-ref="container" id="data">
          <ul class="row g-2">
            ...
            foreach ($cars_data as $index => $item) {
            if (empty($item->country_code)) { $item->country_code = "0"; }

            $title_n = strtolower(str_replace(" ", "-", $item->title));
            $title_name = strtolower(str_replace("&", "", $title_n));

            $link = root.'tour/'.$session_lang.'/'.strtolower($currency).'/'.strtolower(str_replace(' ', '-', $from_location)).'/'. $item->title .'/'.$item->id.'/'.$fromdate.'/'.$item->supplier.'/'.$adults.'/'.$childs;
            if (isset($img)) {
            $hotel_img = $img; } else {
            $hotel_img = root."app/themes/default/assets/img/data/tour.jpg";
            }
            ?>
            <li class="mix stars_ hotels_amenities_ col-6 mt-0" data-a="<?=$item->b2c_price?>" data-b="" id="<?=strtolower($item->title)?>">
              <div class="card-item card-item-list" class="marker-link row row-rtl item hotelslist">
                <div class="card-img p-2" style="width:100px;">
                  <a href="<?=$link?>" class="d-block">
                  <img src="<?php if (!empty($item->thumbnail)){echo $item->thumbnail;} else {echo root."app/themes/default/assets/img/data/tour.jpg";}?>" class="img-fluid" style="min-width:100%" alt="thumbnail" />
                  </a>
                </div>
                <div class="card-body p-0">
                  ...
                  <h3 class="card-title" style="white-space:unset"><?=str_replace('-', ' ', $item->title);?>
                  ...
                  <div class="card-rating pt-0 pb-0">
                    ...
                    <span class="rating__text"><span style="border: 0.4px solid #62a0ff; padding: 6px 11px; border-radius: 18px; margin-right: 8px;"><?=$item->title?></span> <?=T::ratings?></span>
                  </div>
                  </div>
...
$rangeA.ionRangeSlider({
    skin: "round",
    type: "double",
    min: <?php foreach ($cars_data as $index => $item) {$result[$index] = $item->b2c_price;} echo $min_price = min($result); ?>,
    max: <?php foreach ($cars_data as $index => $item) {$result[$index] = $item->b2c_price;} echo $min_price = max($result); ?>,
    from: <?php foreach ($cars_data as $index => $item) {$result[$index] = $item->b2c_price;} echo $min_price = min($result); ?>,
    to: <?php foreach ($cars_data as $index => $item) {$result[$index] = $item->b2c_price;} echo $min_price = max($result); ?>,
