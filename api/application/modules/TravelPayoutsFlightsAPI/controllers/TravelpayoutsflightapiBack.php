...
class TravelpayoutsflightapiBack extends MX_Controller
{
    ...
    public function update_settings()
    {
        ...
        $this->App->service("ModuleService")->update('TravelPayoutsFlightsAPI', 'apiConfig', $payload['apiConfig']);
        redirect(base_url('admin/tpflightapi/settings'));
	}

    public function bookings(){
        $this->load->helper('xcrud');
        $xcrud = xcrud_get_instance();
        $xcrud->table('bookings_travelpayoutsflightapi');
        $xcrud->order_by('id', 'desc');
        $xcrud->columns('price,currency,date,form_city,to_city,url');
        $this->data['add_link'] = '';
        $xcrud->limit(50);
        $xcrud->unset_add();
        $xcrud->unset_view();
        ...
        $this->data['page_title'] = 'Travel Payouts Flight Api Booking Management';
        $this->data['main_content'] = 'temp_view';
        $this->data['header_title'] = 'Travel Payouts Flight Api Booking Management';
        $this->load->view('admin/template', $this->data);
    }
}
