...
class Kiwi extends MX_Controller {

    public function __construct() 
	{
		parent :: __construct();
	
		$chk = modules::run('Home/is_main_module_enabled', 'Kiwi');
		...
		$this->data['appModule'] = "Kiwi";
        $this->load->library('Kiwi/Flights_lib');
        ...
        $this->load->model('Kiwi/FlightsSearchModel');
    }
    ...
    {
         $this->load->model('Kiwi/Flights_model');
        ...
        $this->session->set_userdata('Kiwi',serialize($searchForm));
        ...
        if(($uri != "flights" ) && ($payload[1] != "0")) {
            ...
            $result = $this->Flights_model->get_route($params,0,0,app()->service("ModuleService")->get('kiwi')->test);
            if(empty($result) && app()->service("ModuleService")->get('kiwi')->test == "true")
            {
                $result = $this->Flights_model->get_route($params,0,0,app()->service("ModuleService")->get('kiwi')->test);
            }
			...
			//print_r(app()->service("ModuleService")->get('kiwi_flight')->test);
			
			
			
            $data['result'] = $this->Flights_model->get_route($params, $limit, $offset,app()->service("ModuleService")->get('kiwi')->test);
			
			
		
            $dataset = $this->Flights_model->get_airlines($params,app()->service("ModuleService")->get('kiwi')->test);
            ...
            $dataset = $this->Flights_model->get_airlines_all($payload,app()->service("ModuleService")->get('kiwi')->test);
            $data['result'] = $this->Flights_model->get_route_all($payload,app()->service("ModuleService")->get('kiwi')->test);
        ...
        $this->data["alllinks"] = $data["links"];
       
       // echo '<pre>';
        //print_r($this->data["alllinks"]); die;
				
        $this->theme->view('modules/flights/kiwi/listing', $this->data, $this);
    ...
    function book() {
       ...
       // dd($this->data["tax"] );
        ...
        $this->theme->view('modules/flights/kiwi/booking', $this->data, $this);
