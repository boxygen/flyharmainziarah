...
class FlightsListingModel  extends CI_Model
{
    ...
    function extract_data()
    {
        ...
        foreach ($main_array as $mainindex=>$item) {
             ...
             $segments["segments2"] = array();
             
             $this->db->select("pt_kiwi_flights_routes.*");
             $this->db->where('setting_id',$item->id);
              $this->db->where('returnval',0);
             $allroutes =  $this->db->get('pt_kiwi_flights_routes')->result();
             
            foreach ($allroutes as $item2) 
            { 
                $currency_code = 'USD';
                $departure_time = $item2->time_departure;
                $duration_time = $item->total_hours_departure;
                $duration_time_return = $item->total_hours_return;
                $arrival_time = $item2->time_arrival;
                $departure_date = $item2->date_departure;
                $arrival_date = $item2->date_arrival;
                $class_type = $item2->flight_type;
                $departure_code = '';
                $arrival_code = '';
                $departure_airport = $item2->from_code;
                $arrival_airport = $item2->to_code;
                $flighttype = $item2->type;
		  ...
		  if($item->flight_mode =='return')
		  {
		     $this->db->select("pt_kiwi_flights_routes.*");
             $this->db->where('setting_id',$item->id);
             $this->db->where('returnval',1);
             $allroutes =  $this->db->get('pt_kiwi_flights_routes')->result();
				...
				$form = $this->hidden_parameters($hidden_parameters);
					...
					$bj =  (object)[
						...
						"arrival_code" => $arrival_code,
						"arrival_airport" => $arrival_airport,
						"duration_time" => $duration_time_return,
						"currency_code" => $currency_code,
						"price" => $price,
						"airline_name" => $airline_name->name,
						"booking_class" => $class_type,
        ...
        $this->action_url = site_url('kiwi/book');
    }
