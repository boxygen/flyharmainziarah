...
class IwaystransferBack extends MX_Controller
{
    ...
    public function update_settings()
    {
        ...
        $this->App->service("ModuleService")->update('Iwaystransfer', 'apiConfig', $payload['apiConfig']);
        redirect(base_url('admin/iwaystransfer/settings'));
	}

    public function bookings(){
        $this->load->helper('xcrud');
        $xcrud = xcrud_get_instance();
        $xcrud->table('pt_iwaystransfer_booking');
        $xcrud->order_by('id', 'desc');
        $xcrud->columns('transfer_id,first_name,last_name,email,currency,amount,status,url');
        $this->data['add_link'] = '';
        $xcrud->limit(50);
        $xcrud->unset_add();
        $xcrud->unset_view();
        ...
        $this->data['page_title'] = 'IwaysTransfer Booking Management';
        $this->data['main_content'] = 'temp_view';
        $this->data['table_name'] = 'pt_iwaystransfer_booking';
        $this->data['main_key'] = 'id';
        $this->data['header_title'] = 'IwaysTransfer Booking Management';
        $this->load->view('Admin/template', $this->data);
    }
