...
var cars = scheduler.serverList("room");
...
//   { name:"Email", type:"text", map_to:"email", }
// ];

scheduler.locale.labels.new_event = "Customer Name";
...
scheduler.templates.week_timeline_scale_label =
  ...
  <div class=\"car_brand\">"+label+"</div><div class=\"car_price\">"+section.currency+" "+section.price+"</div></div>";
};
...
scheduler.templates.tooltip_text = function (start, end, event) {
  ...
  var status_string = 'Reserved';
  if (status == 'paid') {
    status_string = 'Booked';
  }
  return status_string + " for " + get_formatted_duration(start, end) + "<br/>" +
      "<b>Status:</b> " + status + "<br/>" +
      "<b>Checkin:</b> " + scheduler.templates.tooltip_date_format(start) + "<br/>" +
      "<b>Checkout:</b> " + scheduler.templates.tooltip_date_format(end) + "<br/>" +
      "<b>Total:</b> " + event.booking_total;
...
scheduler.attachEvent("onEventCollision", function (ev, evs) {
  for (var i = 0; i < evs.length; i++) {
    ...
    dhtmlx.message({
      type: "error",
      text: "This room is already reserved for this date."
    });
  }
  return true;
}); 
...
function filterByType(room, types){
  return !!types[room.type];
}

function filterByPrice(room, prices){
  return prices.some(function(price){
    return +room.price >= price.minVal && +room.price < price.maxVal;
  });
}

function filterByDate(room, dateFrom, dateTo){
  if(dateFrom){
    var evs = scheduler.getEvents();
    
    for (var i=0; i<evs.length; i++){    
      if(room.key == evs[i].room){
...
function updateSections(){
  ...
  var arrByType = cars.filter(function(room){
    return filterByType(room, types)
  });  
  ...
  var arrByPrice = arrByType.filter(function(room){
    return filterByPrice(room, selectedPrices);
  });
  ...
  if(checkbox.checked && newDate){
    ...
    var arrByDate = arrByPrice.filter(function(room){
      return filterByDate(room, targetDateFrom, targetDateTo)
    });
...
scheduler.attachEvent("onXLE", function (){
  ...
  pushCheckboxes("price-options", priceRanges, function(item){ return item.currency+" "+item.minVal+"-"+item.maxVal }, "price");
...
function save_form() {
  ...
  var room = html("lightboxCar").value;
